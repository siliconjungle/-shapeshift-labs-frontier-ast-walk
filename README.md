# @shapeshift-labs/frontier-ast-walk
...
```sh
npm install @shapeshift-labs/frontier-ast-walk
```
...
const graph = walkFrontierSources([
  ...
  },
  {
    ...
    text: "export function HomeView() { return <main />; }"
  }
]);
...
The default layer classifier treats frontend routes/components and backend handlers as adapters. Adapter files are allowed to bind UI, transport, route, and handler surfaces, but effectful calls such as `fetch`, `WebSocket`, `localStorage`, timers, `Date.now`, and `Math.random` are reported as business-logic findings. Domain code belongs in domain roots such as `packages/domain/src` or `packages/contracts/src`.

Frameworks can customize roots, allowed adapter declarations, forbidden calls, and severity through `FrontierAstWalkOptions`.

## API Surface

- `walkFrontierSource(input, options)`: analyze one source string.
- `walkFrontierSources(inputs, options)`: produce a project source graph.
- `classifySourceLayer(file, options)`: map a file into route/component/backend/domain/test/config/generated layers.
- `analyzeBusinessLogic(source, options)`: return adapter/domain policy findings.
- `traceImportClosure(graph, seeds)`: follow local imports from source IDs or files.
- `createAstRegistryGraph(graph)`: convert source facts to a Frontier registry-style graph.
- `createAstLintResources(graph)`: convert source facts to linter-friendly resources.
...
The package benchmark measures source walking and registry conversion over a synthetic multi-file project.
